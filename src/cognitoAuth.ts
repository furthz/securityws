...
var dynamo: DocumentClient = new DynamoDB.DocumentClient({ apiVersion: '2012-08-10', region: REGION })
var poolsDictionary: { [key: string]: ICognito } = {}

const cognitoAuth = async (req: Request, res: Response, next: any) => {
    try {
        //obtener el valor del header client_nexux
        let id_client = req.get(HEADER_CLIENT) || 'soapros'

        const pemsDownloadProm: {[key:string]:string} = await _init(id_client)
...
const getDataClient = async (id_client: string) => {
...
const _verifyMiddleWare = (pem: {[key: string]: string}, req: IAuthenticatedRequest, res: Response, next: any) => {
...
const _verify = (pems: { [key: string]: string }, auth: string, id_client:string): Promise<JwtPayload | string> => {
